...
class Model(object):
    ...
    def init_placeholders(self):
        ...
        self.sl_h = tf.placeholder(tf.int32, [None, ])
        self.sl_f = tf.placeholder(tf.int32, [None, ])
...
#RTL RTL RTL RTL RTL
def attention_net_rtl(enc, sl, dec, num_units, num_heads, num_blocks, dropout_rate, is_training, reuse):
    with tf.variable_scope("all_rtl", reuse=reuse):
        with tf.variable_scope("user_future_group"):
            for i in range(num_blocks):
                with tf.variable_scope("num_blocks_future{}".format(i)):
                    ### Multihead Attention
                    enc, stt_vec = multihead_attention(queries=enc,
                                                       queries_length=sl,
                                                       keys=enc,
                                                       ...
                                                       dropout_rate=dropout_rate,
                                                       is_training=is_training,
                                                       scope="self_attention_rtl"
                                                       )

                    ### Feed Forward
                    enc = feedforward(enc,
                                      num_units=[num_units // 4, num_units],
                                      scope="feed_forward_rtl", reuse=reuse)

        dec = tf.expand_dims(dec, 1)#在1的位置上增加1维
        with tf.variable_scope("item_feature_group_rtl"):
            for i in range(num_blocks):
                with tf.variable_scope("num_blocks_future{}".format(i)):
                    ## Multihead Attention ( vanilla attention)
                    dec, att_vec = multihead_attention(queries=dec,

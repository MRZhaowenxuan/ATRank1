...
tf.app.flags.DEFINE_boolean('time_emb', True, 'Concat time-embedding instead of Add')

# Training parameters
tf.app.flags.DEFINE_boolean('from_scratch', True, 'Romove model_dir, and train from scratch, default: False')
tf.app.flags.DEFINE_string('model_dir', 'save_path_dropout0.5_adam_num_blocks2_dacay0.8_lr0.0001_newdata', 'Path to save model checkpoints')
#随机梯度下降sgd
...
#训练批次32
tf.app.flags.DEFINE_integer('train_batch_size', 100, 'Training Batch size')
#测试批次128
...
tf.app.flags.DEFINE_integer('eval_freq', 50, 'Display training status every this iteration')
...
def train():
    ...
    with open('../atrank/tianchi/dataset_atrank.pkl', 'rb') as f:

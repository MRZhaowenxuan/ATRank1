...
from behavior_prc.input import DataInput, DataInputTest
from behavior_prc.model import Model
...
tf.app.flags.DEFINE_string('model_dir', 'bisIE_adam_blocks2_adam_dropout0.5_lr0.0001_decay0.9998', 'Path to save model checkpoints')
#随机梯度下降sgd
...
def create_model(sess, config, behavior_emb_w):

    # print(json.dumps(config,indent=4),flush=True)
    model = Model(config, behavior_emb_w)
...
def train():
    ...
    with open('../BisIE_mask_prc/u_emb.pkl', 'rb') as f:
        u_emb = pickle.load(f)
    ...
    with sess.as_default():
        ...
        model = create_model(sess, config, u_emb)
